...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        save_as = action(
            get_str("saveAs"), self.save_file_as, "Ctrl+Shift+S", "save-as", get_str("saveAsDetail"), enabled=False
        )

        close = action(get_str("closeCur"), self.close_file, "Ctrl+W", "close", get_str("closeCurDetail"))

        delete_image = action(
            get_str("deleteImg"), self.delete_image, "Ctrl+Shift+D", "close", get_str("deleteImgDetail")
        )

        reset_all = action(get_str("resetAll"), self.reset_all, None, "resetall", get_str("resetAllDetail"))

        color1 = action(
            get_str("boxLineColor"), self.choose_color1, "Ctrl+L", "color_line", get_str("boxLineColorDetail")
        )

        create_mode = action(
            get_str("crtBox"), self.set_create_mode, "w", "new", get_str("crtBoxDetail"), enabled=False
        )
        edit_mode = action(
            get_str("editBox"), self.set_edit_mode, "Ctrl+J", "edit", get_str("editBoxDetail"), enabled=False
        )

        create = action(get_str("crtBox"), self.create_shape, "w", "new", get_str("crtBoxDetail"), enabled=False)
        delete = action(
            get_str("delBox"), self.delete_selected_shape, "Delete", "delete", get_str("delBoxDetail"), enabled=False
        )
        copy = action(
            get_str("dupBox"), self.copy_selected_shape, "Ctrl+D", "copy", get_str("dupBoxDetail"), enabled=False
        )
        ...
        edit = action(
            get_str("editLabel"), self.edit_label, "Ctrl+E", "edit", get_str("editLabelDetail"), enabled=False
        )
        ...
        add_actions(
            self.canvas.menus[1], (action("&Copy here", self.copy_shape), action("&Move here", self.move_shape))
        )
    ...
    def load_file(self, file_path=None):
        ...
        if unicode_file_path and os.path.exists(unicode_file_path):
            ...
            if image.isNull():
                self.error_message(
                    "Error opening file", f"<p>Make sure <i>{unicode_file_path}</i> is a valid image file."
                )
                self.status(f"Error reading {unicode_file_path}")
                return False
...
def get_main_app(argv=None):
    """Standard boilerplate Qt application code. Do everything but app.exec_() -- so that we can test the application in
    one thread.
    """
    if not argv:
        argv = []
